...
import {conmomTableCN, gdTableCN, czTableCN, soltTableCN, zdyTableCN} from './components/table/table.tsx'
...
<ShowSouceCode
  ...
  code={`
    import { Table, Flex, TableColumns} from 'miniEx';
...
const cell4 = (field: any, row) => {
  switch (field) {
    ...
    const App: React.FC = () => (
      return(<Table data={data2} columns={columns2} cell={cell4}></Table>)
    );
